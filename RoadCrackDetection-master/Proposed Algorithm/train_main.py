...
import test  # import 交互式查询.py to get mAP after each epoch
...
# Hyperparameters
hyp = {'lr0': 0.01,  # initial learning rate (SGD=1E-2, Adam=1E-3)  学习率
       'momentum': 0.937,  # SGD momentum  学习率动量
       'weight_decay': 5e-4,  # optimizer weight decay  权重衰减系数
       'giou': 0.05,  # giou loss gain  giou损失的系数
       'cls': 0.58,  # cls loss gain 分类损失的系数
       'cls_pw': 1.0,  # cls BCELoss positive_weight 分类BCELoss中正样本的权重
       'obj': 1.0,  # obj loss gain (*=img_size/320 if img_size != 320) 有无物体损失的系数
       ...
       'hsv_h': 0.014,  # image HSV-Hue augmentation (fraction) 色调
       'hsv_s': 0.68,  # image HSV-Saturation augmentation (fraction) 饱和度
       'hsv_v': 0.36,  # image HSV-Value augmentation (fraction) 明度
       'degrees': 0.0,  # image rotation (+/- deg) 旋转角度
       'translate': 0.0,  # image translation (+/- fraction) 水平和垂直平移
